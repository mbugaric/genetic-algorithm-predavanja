 ...
 * selection is performed in rounds, number of rounds is defined by MATING_POOL_SIZE
 * In each round we take K parents and select the best one and copy it to the pool
 * until we get MATING_POOL_SIZE number of individuals inside 
 * 
...
(function () {
    ...
    function selection(N) {
        return new Promise((resolve, reject) => {
            let matingPool = new Array();
            kTournamentSelectionWithDebug(population, N, K).then(response=>{
                matingPool=response;
                resolve(matingPool);
            })
        });
    ...
    $(a).on('debugSelection', function () {
        if (curr_gen < NUM_GEN) {
            ...
            selection(MATING_POOL_SIZE).then(response=>{
                matingPool = response;
            });
        }
    });
